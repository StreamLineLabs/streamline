...
impl GroupStateDiff {
    ...
    pub fn lag_changes(&self) -> Vec<(String, i32, i64)> {
        self.offset_diffs
            .iter()
            .map(|d| (d.topic.clone(), d.partition, d.lag_delta()))
            .collect()
    }
}
...
impl ConfigDiff {
    pub fn partitions_changed(&self) -> bool {
        self.partition_count_before != self.partition_count_after
    }
    pub fn replication_changed(&self) -> bool {
        self.replication_factor_before != self.replication_factor_after
    }
...
impl SnapshotDiff {
    pub fn summary(&self) -> String {
        let watermark_advances: usize = self
            .watermark_changes
            .values()
            .flat_map(|m| m.values())
...
impl StateDiffEngine {
    pub fn diff(
        ...
        for (group_id, after_snap) in &after.groups {
            if let Some(before_snap) = before.groups.get(group_id) {
                ...
                let partition_reassignments =
                    Self::compute_partition_reassignments(before_snap, after_snap);
    ...
    fn compute_offset_diffs(before: &GroupSnapshot, after: &GroupSnapshot) -> Vec<OffsetDiff> {
        let all_keys: HashSet<&String> =
            before.offsets.keys().chain(after.offsets.keys()).collect();
    ...
    fn compute_member_changes(before: &GroupSnapshot, after: &GroupSnapshot) -> Vec<MemberChange> {
        ...
        for m in &after.members {
            if !before_set.contains(m) {
                changes.push(MemberChange {
                    member_id: m.clone(),
                    change_type: MemberChangeType::Added,
                });
            }
        }
        for m in &before.members {
            if !after_set.contains(m) {
                changes.push(MemberChange {
                    member_id: m.clone(),
                    change_type: MemberChangeType::Removed,
                });
            }
        }
        changes
    ...
    fn compute_partition_reassignments(
        before: &GroupSnapshot,
        after: &GroupSnapshot,
    ) -> Vec<PartitionReassignment> {
        let all_keys: HashSet<&String> = before
            .assignments
            .keys()
            .chain(after.assignments.keys())
            .collect();
        ...
        for key in all_keys {
            ...
            if prev != next {
                ...
                reassignments.push(PartitionReassignment {
                    topic,
                    partition,
                    previous_owner: prev.cloned(),
                    new_owner: next.cloned(),
                });
            }
        }
        reassignments
    ...
    pub fn diff_consumer_groups(
        ...
        GroupStateDiff {
            ...
            members_before: before.member_count,
            members_after: after.member_count,
            member_changes,
            offset_diffs,
            partition_reassignments,
        }
    ...
    pub fn diff_topic_configs(
        before: &TopicConfigSnapshot,
        after: &TopicConfigSnapshot,
    ) -> ConfigDiff {
        let all_keys: HashSet<&String> =
            before.configs.keys().chain(after.configs.keys()).collect();
        let mut changes = Vec::new();
        for key in all_keys {
            match (before.configs.get(key), after.configs.get(key)) {
                (Some(vb), Some(va)) if vb != va => changes.push(ConfigChange {
                    key: key.to_string(),
                    value_before: Some(vb.clone()),
                    value_after: Some(va.clone()),
        ...
        ConfigDiff {
            ...
            partition_count_after: after.partition_count,
            replication_factor_before: before.replication_factor,
            replication_factor_after: after.replication_factor,
    ...
    pub fn compare_snapshots(before: &DebugSnapshot, after: &DebugSnapshot) -> SnapshotDiff {
        ...
        let topics_added: Vec<String> = after_topics
            .difference(&before_topics)
            .map(|t| (*t).clone())
            .collect();
        let topics_removed: Vec<String> = before_topics
            .difference(&after_topics)
            .map(|t| (*t).clone())
            .collect();
        let mut config_diffs = Vec::new();
        for topic in before_topics.intersection(&after_topics) {
            if let (Some(bc), Some(ac)) = (
                before.topic_configs.get(*topic),
                after.topic_configs.get(*topic),
            ) {
                let diff = Self::diff_topic_configs(bc, ac);
                if diff.has_config_changes()
                    || diff.partitions_changed()
                    || diff.replication_changed()
                {
                    config_diffs.push(diff);
                }
            }
        }
        let all_hw_topics: HashSet<&String> = before
            .high_watermarks
            .keys()
            .chain(after.high_watermarks.keys())
            .collect();
        ...
        for topic in all_hw_topics {
            ...
            let all_partitions: HashSet<i32> = before_hw
                .map(|m| m.keys().copied().collect::<HashSet<_>>())
                .unwrap_or_default()
                .union(
                    &after_hw
                        .map(|m| m.keys().copied().collect::<HashSet<_>>())
            ...
            for p in all_partitions {
                ...
                if bw != aw {
                    partition_changes.insert(p, (bw, aw));
                }
            }
...
mod tests {
    ...
    fn test_state_diff_groups_added_removed() {
        let before = ConsumerStateSnapshot {
            ...
            groups: {
                ...
                m.insert(
                    "old-group".to_string(),
                    make_group_snapshot("Stable", vec!["m1"], vec![]),
                );
                m
            },
        };
    ...
    fn test_member_changes() {
        ...
        assert!(diff
            .member_changes
            .iter()
            .any(|c| c.member_id == "m3" && c.change_type == MemberChangeType::Added));
        assert!(diff
            .member_changes
            .iter()
            .any(|c| c.member_id == "m1" && c.change_type == MemberChangeType::Removed));
    }
    ...
    fn test_partition_reassignments() {
        ...
        before
            .assignments
            .insert("t:0".to_string(), "m1".to_string());
        before
            .assignments
            .insert("t:1".to_string(), "m2".to_string());
        let mut after = make_group_snapshot("Stable", vec!["m1", "m2"], vec![]);
        after
            .assignments
            .insert("t:0".to_string(), "m2".to_string());
        after
            .assignments
            .insert("t:1".to_string(), "m1".to_string());
        let diff = StateDiffEngine::diff_consumer_groups(&before, &after, "g1");
    ...
    fn test_create_and_compare_snapshots() {
        let before = StateDiffEngine::create_snapshot(
            "t1",
            1000,
            ConsumerStateSnapshot {
                ...
                groups: {
                    ...
                    m.insert(
                        "g1".to_string(),
                        make_group_snapshot("Stable", vec!["m1"], vec![("t:0", 10, 20)]),
                    );
                    m
                },
            },
        ...
        let after = StateDiffEngine::create_snapshot(
            "t2",
            2000,
            ConsumerStateSnapshot {
                timestamp_ms: 2000,
                groups: {
                    ...
                    m.insert(
                        "g1".to_string(),
                        make_group_snapshot("Stable", vec!["m1"], vec![("t:0", 18, 30)]),
                    );
                    m
                },
            },
