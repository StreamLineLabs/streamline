...
pub enum BreakpointCondition {
    ...
    SizeThreshold { min_bytes: Option<usize>, max_bytes: Option<usize> },
    /// Combine multiple conditions with a logical operator.
    Composite { conditions: Vec<BreakpointCondition>, op: LogicalOp },
}

/// Evaluate a [`BreakpointCondition`] against a record and its partition context.
pub fn evaluate_condition(condition: &BreakpointCondition, record: &Record, partition: i32) -> bool {
    match condition {
        BreakpointCondition::OnKey { pattern } => {
            let Some(key) = record.key.as_ref() else { return false };
            let Ok(key_str) = std::str::from_utf8(key) else { return false };
            Regex::new(pattern).map(|re| re.is_match(key_str)).unwrap_or(false)
        }
        BreakpointCondition::OnValue { pattern } => {
            let Ok(val_str) = std::str::from_utf8(&record.value) else { return false };
            Regex::new(pattern).map(|re| re.is_match(val_str)).unwrap_or(false)
        }
        BreakpointCondition::OnHeader { key, value } => record.headers.iter().any(|h| {
            if h.key != *key { return false; }
            match value {
                Some(expected) => {
                    let Ok(hv) = std::str::from_utf8(&h.value) else { return false };
                    Regex::new(expected).map(|re| re.is_match(hv)).unwrap_or(false)
                }
                None => true,
            }
        }),
        ...
        BreakpointCondition::JsonField { path, value: expected } => {
            let Ok(json) = serde_json::from_slice::<serde_json::Value>(&record.value) else { return false };
        ...
        BreakpointCondition::SizeThreshold { min_bytes, max_bytes } => {
        ...
        BreakpointCondition::Composite { conditions, op } => match op {
            LogicalOp::And => conditions.iter().all(|c| evaluate_condition(c, record, partition)),
            LogicalOp::Or => conditions.iter().any(|c| evaluate_condition(c, record, partition)),
        },
    }
}
...
pub enum BreakpointType {
    ...
    SizeThreshold { min_bytes: Option<usize>, max_bytes: Option<usize> },
    TimeRange { start_ms: Option<i64>, end_ms: Option<i64> },
    ConditionOnly,
}
...
impl Breakpoint {
    ...
    pub fn pattern(pattern: impl Into<String>) -> Self {
        ...
        Self {
            id, name: format!("pattern-{}", id), enabled: true,
            breakpoint_type: BreakpointType::Pattern(PatternBreakpoint {
                pattern: pattern.into(), case_insensitive: false, search_keys: false,
            }),
            condition: None, action: BreakpointAction::Pause, hit_count: 0,
        }
    }
    ...
    pub fn at_offset(offset: i64) -> Self {
        let id = next_breakpoint_id();
        Self {
            id, name: format!("offset-{}", offset), enabled: true,
    ...
    pub fn json_field(path: impl Into<String>, value: impl Into<String>) -> Self {
        let id = next_breakpoint_id();
        Self {
            id, name: format!("json-{}", id), enabled: true,
            breakpoint_type: BreakpointType::JsonField { path: path.into(), value: value.into() },
            condition: None, action: BreakpointAction::Pause, hit_count: 0,
        }
    }
    ...
    pub fn from_condition(name: impl Into<String>, condition: BreakpointCondition) -> Self {
        let id = next_breakpoint_id();
        Self {
            id, name: name.into(), enabled: true,
            breakpoint_type: BreakpointType::ConditionOnly,
            condition: Some(condition), action: BreakpointAction::Pause, hit_count: 0,
        }
    }
    ...
    pub fn matches(&self, offset: i64, key: Option<&[u8]>, value: Option<&[u8]>) -> bool {
        ...
        match &self.breakpoint_type {
            ...
            BreakpointType::JsonField { path, value: expected } => {
                if let Some(v) = value {
            ...
            BreakpointType::KeyMatch { pattern } => {
                if let Some(k) = key {
                    if let Ok(s) = std::str::from_utf8(k) {
                        return Regex::new(pattern).map(|re| re.is_match(s)).unwrap_or(false);
                    }
                }
                false
            }
            BreakpointType::Header { .. } => false,
            BreakpointType::SizeThreshold { min_bytes, max_bytes } => {
...
impl BreakpointManager {
    pub fn new() -> Self {
        Self { breakpoints: Vec::new(), max_hits_per_second: 0, rate_state: HashMap::new() }
    }

    pub fn with_rate_limit(max_hits_per_second: u64) -> Self {
        Self { breakpoints: Vec::new(), max_hits_per_second, rate_state: HashMap::new() }
    }
    ...
    pub fn reset_counters(&mut self) {
        for bp in &mut self.breakpoints { bp.hit_count = 0; }
    ...
    pub fn evaluate_all(&mut self, record: &Record, partition: i32) -> Vec<BreakpointMatch> {
        ...
        for bp in &mut self.breakpoints {
            if !bp.evaluate(record, partition) { continue; }
            if self.max_hits_per_second > 0 {
                let state = self.rate_state.entry(bp.id).or_insert(RateLimiterState {
                    window_start: now, hits_in_window: 0,
                });
                let elapsed = now.duration_since(state.window_start);
                if elapsed.as_secs() >= 1 { state.window_start = now; state.hits_in_window = 0; }
                if state.hits_in_window >= self.max_hits_per_second { continue; }
            ...
            matches.push(BreakpointMatch {
                ...
                matched_content: content, offset: record.offset, partition,
...
mod tests {
    ...
    fn make_record(offset: i64, key: Option<&str>, value: &[u8]) -> Record {
        Record {
            ...
            headers: Vec::new(), crc: None,
        }
    }

    fn make_record_with_headers(offset: i64, value: &[u8], headers: Vec<(&str, &[u8])>) -> Record {
        Record {
            offset, timestamp: 1500, key: None,
            value: Bytes::from(value.to_vec()),
            headers: headers.into_iter().map(|(k, v)| Header {
                key: k.to_string(), value: Bytes::from(v.to_vec()),
            }).collect(),
            crc: None,
        }
    }
    ...
    fn test_size_threshold_breakpoint() {
        let bp = Breakpoint {
            id: 1, name: "size".to_string(), enabled: true,
            breakpoint_type: BreakpointType::SizeThreshold { min_bytes: Some(10), max_bytes: None },
            condition: None, action: BreakpointAction::Pause, hit_count: 0,
        };
    ...
    fn test_condition_on_key() {
        let cond = BreakpointCondition::OnKey { pattern: "^user-".to_string() };
    ...
    fn test_condition_on_value() {
        let cond = BreakpointCondition::OnValue { pattern: "error".to_string() };
    ...
    fn test_condition_on_header() {
        let cond = BreakpointCondition::OnHeader {
            key: "content-type".to_string(), value: Some("json".to_string()),
        };
        let record = make_record_with_headers(0, b"data", vec![("content-type", b"application/json")]);
        assert!(evaluate_condition(&cond, &record, 0));
    ...
    fn test_condition_on_offset_range() {
        ...
        assert!(evaluate_condition(&cond, &make_record(15, None, b"data"), 0));
        assert!(!evaluate_condition(&cond, &make_record(25, None, b"data"), 0));
        assert!(evaluate_condition(&cond, &make_record(10, None, b"data"), 0));
        assert!(evaluate_condition(&cond, &make_record(20, None, b"data"), 0));
    }

    #[test]
    fn test_condition_on_timestamp_range() {
        let cond = BreakpointCondition::OnTimestamp { start_ms: 1000, end_ms: 2000 };
        let record = make_record(0, None, b"data");
        assert!(evaluate_condition(&cond, &record, 0));
    ...
    fn test_breakpoint_from_condition() {
        let bp = Breakpoint::from_condition("error-on-p0", BreakpointCondition::Composite {
            conditions: vec![
                BreakpointCondition::OnPartition { id: 0 },
                BreakpointCondition::OnValue { pattern: "error".to_string() },
            ],
    ...
    fn test_manager_evaluate_all() {
        let mut mgr = BreakpointManager::new();
        mgr.add(Breakpoint::from_condition("errors", BreakpointCondition::OnValue { pattern: "error".to_string() }));
        mgr.add(Breakpoint::from_condition("partition-0", BreakpointCondition::OnPartition { id: 0 }));
        let record = make_record(0, None, b"an error occurred");
    ...
    fn test_manager_hit_counting() {
        let mut mgr = BreakpointManager::new();
        let id = mgr.add(Breakpoint::from_condition("counter", BreakpointCondition::OnValue { pattern: "hit".to_string() }));
    ...
    fn test_manager_rate_limiting() {
        ...
        mgr.add(Breakpoint::from_condition("limited", BreakpointCondition::OnValue { pattern: "hit".to_string() }));
        let record = make_record(0, None, b"hit me");
        ...
        let cond = BreakpointCondition::Composite {
            conditions: vec![
                ...
                BreakpointCondition::Composite {
                    conditions: vec![
                        BreakpointCondition::OnValue { pattern: "error".to_string() },
                        BreakpointCondition::OnValue { pattern: "warning".to_string() },
                    ],
                    op: LogicalOp::Or,
                },
            ],
