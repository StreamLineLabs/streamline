...
pub use breakpoint::{
    evaluate_condition, Breakpoint, BreakpointAction, BreakpointCondition, BreakpointManager,
    BreakpointMatch, BreakpointType, LogicalOp, PatternBreakpoint,
};
pub use inspector::{
    detect_format, diff_messages, inspect_message, trace_message, ChangeType, DebuggerConfig,
    DetectedFormat, EventView, FieldDiff, HeaderAnalysis, InspectionResult, MessageDiff,
    MessageInspection, MessageInspector, MessageTrace, NavigationDirection, SizeBreakdown,
};
pub use state_diff::{
...
impl StreamDebugger {
    ...
    pub fn with_rate_limit(
        ...
        Self {
            topic_manager,
            breakpoint_manager: BreakpointManager::with_rate_limit(max_hits_per_second),
            targets: HashMap::new(),
            snapshots: Vec::new(),
            session_log: Vec::new(),
            config,
    ...
    pub fn attach_topic(&mut self, topic: impl Into<String>, partition: i32) {
        ...
        self.log_event(
            format!("Attached to {topic}:{partition}"),
            Some(&topic),
            Some(partition),
            None,
        );
    ...
    pub fn detach_topic(&mut self, topic: &str, partition: i32) -> bool {
        ...
        if removed {
            self.log_event(
                format!("Detached from {topic}:{partition}"),
                Some(topic),
                Some(partition),
                None,
            );
    ...
    pub fn attached_topics(&self) -> Vec<(String, i32)> {
        self.targets
            .keys()
            .map(|t| (t.topic.clone(), t.partition))
            .collect()
    }

    pub fn is_attached(&self, topic: &str, partition: i32) -> bool {
        self.targets.contains_key(&DebugTarget {
            topic: topic.to_string(),
            partition,
        })
    }
    ...
    pub fn remove_breakpoint(&mut self, id: u64) -> bool {
        ...
        if removed {
            self.log_event(format!("Removed breakpoint {id}"), None, None, None);
        }
        removed
    }

    pub fn enable_breakpoint(&mut self, id: u64) -> bool {
        self.breakpoint_manager.enable(id)
    }
    pub fn disable_breakpoint(&mut self, id: u64) -> bool {
        self.breakpoint_manager.disable(id)
    }
    pub fn breakpoints(&self) -> &[Breakpoint] {
        self.breakpoint_manager.list()
    }
    ...
    pub fn step(&mut self, topic: &str, partition: i32) -> Result<StepResult> {
        ...
        self.log_event(
            format!("Step to offset {}", record.offset),
            Some(topic),
            Some(partition),
            Some(record.offset),
        );
        Ok(StepResult {
            record: Some(event_view),
            matches,
            paused: true,
        })
    }
    ...
    pub fn continue_(&mut self, topic: &str, partition: i32) -> Result<StepResult> {
        ...
        while offset < latest {
            ...
            for record in &records {
                ...
                let should_pause = matches
                    .iter()
                    .any(|m| matches!(m.action, BreakpointAction::Pause));
                if should_pause {
                    let event_view =
                        EventView::from_record(topic, partition, record, self.config.pretty_json);
                    self.targets.insert(
                        target,
                        DebugState::Paused {
                            offset: record.offset,
                    ...
                    self.log_event(
                        format!("Paused at offset {}", record.offset),
                        Some(topic),
                        Some(partition),
                        Some(record.offset),
                    );
        ...
        self.targets.insert(
            target,
            DebugState::Paused {
                offset: latest.saturating_sub(1),
            },
        );
        self.log_event(
            "Reached end of topic".to_string(),
            Some(topic),
            Some(partition),
            None,
        );
    ...
    pub fn replay(
        ...
        while offset <= to_offset {
            ...
            for record in &records {
                ...
                events.push(EventView::from_record(
                    topic,
                    partition,
                    record,
    ...
    pub fn inspect_at(
        ...
        partition: i32,
        offset: i64,
    ) -> Result<Option<MessageInspection>> {
    ...
    pub fn capture_snapshot(
        ...
        for target in self.targets.keys() {
            let hw = self
                .topic_manager
                .latest_offset(&target.topic, target.partition)
                .unwrap_or(0);
            high_watermarks
                .entry(target.topic.clone())
                .or_default()
                .insert(target.partition, hw);
        }
        let timestamp_ms = chrono::Utc::now().timestamp_millis();
        let snapshot = StateDiffEngine::create_snapshot(
            &label,
            timestamp_ms,
            consumer_state,
            topic_configs,
        ...
        self.log_event(
            format!("Captured snapshot '{label}' (index {idx})"),
            None,
            None,
            None,
        );
    ...
    pub fn get_snapshot(&self, idx: usize) -> Option<&DebugSnapshot> {
...
mod tests {
    ...
    fn setup_instance(topic: &str, count: usize) -> EmbeddedStreamline {
        ...
        for i in 0..count {
            let msg = if i == 5 {
                ...
                format!(r#"{{"level":"info","seq":{i}}}"#)
            };
            instance
                .produce(topic, 0, None, Bytes::from(msg))
                .expect("produce");
        }
        instance
    }
    ...
    fn test_debugger_attach_detach() {
        ...
        let mut dbg =
            StreamDebugger::new(instance.topic_manager().clone(), DebuggerConfig::default());
    ...
    fn test_debugger_continue_to_breakpoint() {
        ...
        dbg.add_breakpoint(Breakpoint::from_condition(
            "error-bp",
    ...
    fn test_debugger_diff_at() {
        ...
        let dbg = StreamDebugger::new(instance.topic_manager().clone(), DebuggerConfig::default());
        let diff = dbg
            .diff_at("diff-test", 0, 0, 1)
            .expect("diff")
            .expect("both records found");
    ...
    fn test_debugger_snapshot_and_compare() {
        ...
        let state1 = ConsumerStateSnapshot {
            timestamp_ms: 1000,
            groups: HashMap::new(),
        };
        let idx1 = dbg.capture_snapshot("snapshot-1", state1, HashMap::new());
        let state2 = ConsumerStateSnapshot {
            timestamp_ms: 2000,
            groups: HashMap::new(),
        };
            ...
            .produce_with_headers(
                ...
                vec![
                    Header {
                        ...
                        value: Bytes::from("corr-1"),
                    },
                    Header {
                        key: "source".to_string(),
                        value: Bytes::from("svc-a"),
                    },
                ],
            )
            .expect("produce");
