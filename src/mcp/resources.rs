    ...
    if let Ok(topics) = backend.list_topics().await {
        for topic in &topics {
            resources.push(ResourceDefinition {
                ...
                description: Some(format!("Topic '{}' ({} partitions, {} messages)", topic.name, topic.partitions, topic.total_messages)),
                mime_type: Some("application/json".to_string()),
            });
        }
    }
    resources.push(ResourceDefinition { uri: "streamline://topics".to_string(), name: "All Topics".to_string(), description: Some("Overview of all topics".to_string()), mime_type: Some("application/json".to_string()) });
    resources.push(ResourceDefinition { uri: "streamline://server/info".to_string(), name: "Server Info".to_string(), description: Some("Streamline server information".to_string()), mime_type: Some("application/json".to_string()) });
    resources.push(ResourceDefinition { uri: "streamline://metrics".to_string(), name: "Metrics".to_string(), description: Some("Server metrics snapshot".to_string()), mime_type: Some("application/json".to_string()) });
    resources.push(ResourceDefinition { uri: "streamline://config".to_string(), name: "Configuration".to_string(), description: Some("Current server configuration (secrets redacted)".to_string()), mime_type: Some("application/json".to_string()) });
    Ok(resources)
}
...
pub fn list_resource_templates() -> Vec<ResourceTemplate> {
    vec![
        ResourceTemplate { uri_template: "streamline://topics/{topic}".to_string(), name: "Topic Details".to_string(), description: Some("Get metadata for a specific topic".to_string()), mime_type: Some("application/json".to_string()) },
        ResourceTemplate { uri_template: "streamline://topics/{topic}/partitions/{partition}".to_string(), name: "Partition Details".to_string(), description: Some("Get partition information".to_string()), mime_type: Some("application/json".to_string()) },
    ]
}

/// Read a specific resource by URI
pub async fn read_resource(uri: &str, backend: &dyn McpBackend) -> McpResult<Vec<ResourceContent>> {
    let path = uri.strip_prefix("streamline://")
    ...
    match parts.as_slice() {
        ...
        ["topics", topic_name, "partitions", partition] => {
            let p: i32 = partition.parse().map_err(|_| McpError::invalid_params("Invalid partition number"))?;
...
async fn read_server_info(backend: &dyn McpBackend) -> McpResult<Vec<ResourceContent>> {
    ...
    Ok(vec![ResourceContent { uri: "streamline://server/info".to_string(), mime_type: Some("application/json".to_string()), text: Some(json_pretty(&info)?), blob: None }])
}
...
async fn read_topics_overview(backend: &dyn McpBackend) -> McpResult<Vec<ResourceContent>> {
    ...
    Ok(vec![ResourceContent { uri: "streamline://topics".to_string(), mime_type: Some("application/json".to_string()), text: Some(json_pretty(&result)?), blob: None }])
}

async fn read_topic_detail(topic_name: &str, backend: &dyn McpBackend) -> McpResult<Vec<ResourceContent>> {
    let detail = backend.describe_topic(topic_name).await?;
    let sample = if !detail.partitions.is_empty() { backend.consume(topic_name, 0, -1, 5).await.unwrap_or_default() } else { Vec::new() };
    ...
    Ok(vec![ResourceContent { uri: format!("streamline://topics/{}", topic_name), mime_type: Some("application/json".to_string()), text: Some(json_pretty(&result)?), blob: None }])
}

async fn read_partition_detail(topic_name: &str, partition: i32, backend: &dyn McpBackend) -> McpResult<Vec<ResourceContent>> {
    let detail = backend.describe_topic(topic_name).await?;
    let part = detail.partitions.iter().find(|p| p.id == partition)
    ...
    Ok(vec![ResourceContent { uri: format!("streamline://topics/{}/partitions/{}", topic_name, partition), mime_type: Some("application/json".to_string()), text: Some(json_pretty(&result)?), blob: None }])
}
...
async fn read_metrics(backend: &dyn McpBackend) -> McpResult<Vec<ResourceContent>> {
    ...
    Ok(vec![ResourceContent { uri: "streamline://metrics".to_string(), mime_type: Some("application/json".to_string()), text: Some(json_pretty(&result)?), blob: None }])
}
...
async fn read_config(backend: &dyn McpBackend) -> McpResult<Vec<ResourceContent>> {
    ...
    Ok(vec![ResourceContent { uri: "streamline://config".to_string(), mime_type: Some("application/json".to_string()), text: Some(json_pretty(&result)?), blob: None }])
}
...
mod tests {
    ...
    fn make_mock_with_data() -> MockMcpBackend {
        ...
        mock.add_messages("events", vec![
            MessageInfo { offset: 0, timestamp: 1700000000000, key: Some("k1".to_string()), value: "event-data-1".to_string(), headers: vec![] },
            MessageInfo { offset: 1, timestamp: 1700000001000, key: None, value: "event-data-2".to_string(), headers: vec![] },
        ]);
        mock
    }
    ...
    async fn test_read_server_info() {
        let mock = MockMcpBackend::new();
        let contents = read_resource("streamline://server/info", &mock).await.unwrap();
    ...
    async fn test_read_topic_detail() {
        let mock = make_mock_with_data();
        let contents = read_resource("streamline://topics/events", &mock).await.unwrap();
        assert_eq!(contents.len(), 1);
    ...
    async fn test_read_topic_detail_not_found() {
        let mock = MockMcpBackend::new();
        let err = read_resource("streamline://topics/nope", &mock).await.unwrap_err();
        assert_eq!(err.code, crate::mcp::McpErrorCode::TopicNotFound);
    }

    #[tokio::test]
    async fn test_read_partition_detail() {
        let mock = make_mock_with_data();
        let contents = read_resource("streamline://topics/events/partitions/0", &mock).await.unwrap();
        assert_eq!(contents.len(), 1);
        let text = contents[0].text.as_ref().unwrap();
    ...
    async fn test_read_partition_not_found() {
        let mock = make_mock_with_data();
        let err = read_resource("streamline://topics/events/partitions/99", &mock).await.unwrap_err();
        assert_eq!(err.code, crate::mcp::McpErrorCode::PartitionNotFound);
    }
    ...
    async fn test_read_unknown_path() {
        let mock = MockMcpBackend::new();
        let err = read_resource("streamline://unknown/path", &mock).await.unwrap_err();
        assert_eq!(err.code, crate::mcp::McpErrorCode::ResourceNotFound);
    }

    #[tokio::test]
    async fn test_read_invalid_partition_number() {
        let mock = make_mock_with_data();
        let err = read_resource("streamline://topics/events/partitions/abc", &mock).await.unwrap_err();
        assert_eq!(err.code, crate::mcp::McpErrorCode::InvalidParams);
    }
}
