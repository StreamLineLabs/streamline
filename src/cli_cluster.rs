...
pub(super) fn init_cluster(
    ...
    if nodes % 2 == 0 {
        ctx.warn(&format!(
            "Even number of nodes ({nodes}). Odd numbers (3, 5, 7) are recommended for consensus."
        ));
    }
    ...
    let spinner = ctx.spinner(&format!(
        "Initializing {nodes}-node cluster configuration..."
    ));
    ...
    for i in 1..=nodes {
        ...
        let env_path = output_dir.join(format!("node{node_id}.env"));
        std::fs::write(&env_path, &env_content)?;
        created_files.push(env_path.display().to_string());

        // Generate shell script
        let script_content = generate_start_script(node_id, &env_path);
        let script_path = output_dir.join(format!("start-node{node_id}.sh"));
...
fn generate_env_file(
    ...
    let mut env = format!(
        r#"# Streamline Node {node_id} Configuration
# Generated by streamline-cli cluster init

# Node identity
STREAMLINE_NODE_ID={node_id}

# Network addresses
STREAMLINE_LISTEN_ADDR=0.0.0.0:{kafka_port}
STREAMLINE_ADVERTISED_ADDR={host}:{kafka_port}
STREAMLINE_INTER_BROKER_ADDR=0.0.0.0:{cluster_port}
STREAMLINE_HTTP_ADDR=0.0.0.0:{http_port}

# Data directory
STREAMLINE_DATA_DIR=./data/node{node_id}
...
"#,
  ...
  streamline-node{i}:
    ...
    container_name: streamline-node{i}
    hostname: streamline-node{i}
    ports:
      - "{node_kafka_port}:{node_kafka_port}"
      - "{node_cluster_port}:{node_cluster_port}"
      - "{node_http_port}:{node_http_port}"
    environment:
      - STREAMLINE_NODE_ID={i}
      - STREAMLINE_LISTEN_ADDR=0.0.0.0:{node_kafka_port}
      - STREAMLINE_ADVERTISED_ADDR=streamline-node{i}:{node_kafka_port}
      - STREAMLINE_INTER_BROKER_ADDR=0.0.0.0:{node_cluster_port}
      - STREAMLINE_HTTP_ADDR=0.0.0.0:{node_http_port}
...
"#,
    ...
    for i in 1..=nodes {
        compose.push_str(&format!("  streamline-data-{i}:\n"));
    }
...
This directory contains configuration files for a {nodes}-node Streamline cluster.
...
"#,
    ...
    for i in 1..=nodes {
        let node_kafka_port = kafka_port + ((i - 1) * 10) as u16;
        readme.push_str(&format!("- `{host}:{node_kafka_port}`\n"));
    }
...
"#,
    ...
    for i in 1..=nodes {
        let node_http_port = http_port + ((i - 1) * 10) as u16;
        readme.push_str(&format!(
            "```bash\ncurl http://{host}:{node_http_port}/health\n```\n"
        ));
    }
