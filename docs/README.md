# Documentation Index

## Getting Started
- [**DEVELOPMENT.md**](DEVELOPMENT.md) — Clone to productive in 5 minutes
- [**TROUBLESHOOTING.md**](TROUBLESHOOTING.md) — Common issues and solutions
- [**KAFKA_MIGRATION.md**](KAFKA_MIGRATION.md) — Migrating from Apache Kafka

## Architecture & Design
- [**ARCHITECTURE.md**](ARCHITECTURE.md) — System overview, module map, data flows
- [**API_STABILITY.md**](API_STABILITY.md) — Module stability tiers, Kafka compat matrix, LTS policy
- [**ADRs**](adr/) — 10 Architecture Decision Records

## Performance
- [**BENCHMARKS.md**](BENCHMARKS.md) — Methodology, results, reproduction instructions

## Release & Quality
- [**V1_RELEASE_CRITERIA.md**](V1_RELEASE_CRITERIA.md) — 48 checkboxes for v1.0 readiness
- [**LAUNCH_CHECKLIST.md**](LAUNCH_CHECKLIST.md) — Registry publication and community launch

## Code Quality Guides
- [**UNWRAP_MIGRATION.md**](UNWRAP_MIGRATION.md) — How to fix quarantined unwrap() calls
- [**DEAD_CODE_AUDIT.md**](DEAD_CODE_AUDIT.md) — Tracking unused code annotations
- [**STORAGE_REFACTORING.md**](STORAGE_REFACTORING.md) — Plan for decomposing large storage files

## Operations & Deployment
- [Clustering Operations](CLUSTERING_OPERATIONS.md) — failure modes, rolling upgrades, autoscaling

## Compliance & Security
- [Compliance & Security](COMPLIANCE.md) — SOC2 gap analysis, security posture
- [GDPR Compliance](GDPR.md) — data residency, right-to-erasure, encryption

## Integrations
- [Kafka Connect Compatibility](KAFKA_CONNECT_COMPATIBILITY.md) — REST API, connector testing matrix

## Domain-Specific
- [analytics.md](analytics.md) — DuckDB analytics engine
- [clustering.md](clustering.md) — Raft-based clustering
- [pubsub.md](pubsub.md) — Redis-compatible pub/sub messaging
- [transactions.md](transactions.md) — Exactly-once semantics
